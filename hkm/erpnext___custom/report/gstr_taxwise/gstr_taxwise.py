...
def get_invoices(filters):
    ...
    for (
        ...
        TAXABLE = taxable
        item_taxes = []
        item_code = item_name if not item_code else item_code
        if (invoice_no in inv_item_tax_map) and (
            item_code in inv_item_tax_map[invoice_no]
        ):
            item_taxes = inv_item_tax_map[invoice_no][item_code]
